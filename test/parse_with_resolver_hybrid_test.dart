@TestOn('vm')
library parse_with_resolver_hybrid_test;
...
void main() {
  group('resolver: hybrid (file + memory)', () {
    test(
      'tail after first include cannot override include keys (hybrid)',
      () async {
    ...
    test(
      'relative include resolves against including file directory',
      () async {
        final tempDir = await Directory.systemTemp.createTemp(
          'flatconfig_test_',
        );
    ...
    test('file base includes + memory override', () async {
      ...
      try {
        ...
        File(
          p.join(tempDir.path, 'colors.conf'),
    ...
    test(
      'hybrid: reset in main does not block later include reassignment',
      () async {
